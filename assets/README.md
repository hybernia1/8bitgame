...
This project will generate procedural sprites when no external textures are present. To override them, drop your own **32x32 PNG** textures into the folders below (leave files out if you want to keep the procedural look). Create the folder if it is missing (props/items are trimmed by default to keep the repo light). Preferred locations are:
