...
- Destroy overlays: `D1`, `W1D1` (overlay + base in one token). Use `W#D#` for destructible walls; the transparent destroy sprite partially covers the chosen wall variant and replaces the old `WC` token.
